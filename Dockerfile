...
RUN apt update && apt install -y g++ g++-multilib git gdb make
#RUN chsh -s /bin/bash 
...
WORKDIR /home
RUN git clone https://github.com/tymm/Rosette-VM

WORKDIR Rosette-VM

#RUN sed -i '2s/.*/\tgdb ..\/..\/bin\/boot-ess #-boot boot.rbl/' rbl/rosette/Makefile


#ENTRYPOINT make CFLAGS=-g
